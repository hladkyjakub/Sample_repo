            ...
            wml.tag.row {
                wml.tag.column {
                    ...
                    wml.tag.toggle_panel {
						definition="narration_toggle_panel",
                        listboxItem
                    }
                }
            }
...
-- }
	...
	local narration_toggle_panel = wml.load("~add-ons/Bandits_from_Brown_Hills/my_toggle_panel.cfg")
	gui.add_widget_definition("toggle_panel", "narration_toggle_panel", wml.get_child(narration_toggle_panel, 'toggle_panel_definition'))
	...
	local narration_button = wml.load("~add-ons/Bandits_from_Brown_Hills/narration_button.cfg")
	gui.add_widget_definition("button", "narration_button", wml.get_child(narration_button, 'button_definition'))
	...
	local function narration_preshow(dialog)
        ...
        for i, monster in ipairs(monsters) do
            ...
            listbox_label[i]= "<span size='"..font_size_listbox.."' font_family='"..font_family_listbox.."' color='"..color_listbox.."' >".."~ "..monster.string.."</span>"
			listbox[i].button.label = listbox_label[i] --From button back to label, changete on_button_click to on_double_click
			listbox[i].button.on_button_click= function()
				local t = wesnoth.ms_since_init()
				std_print("BUTTON: "..i.." was selected, In time of: "..t)
				if last_click ~= -1 and last_click + 400 > t and last_selected == i then
					std_print("option: "..i.." was selected twice, end dialog now")
				else
					last_click = t
					last_selected = i
				end
				local i2 = 1
				while listbox_label[i2] ~= nil do
					listbox[i2].button.label = listbox_label[i2]
					i2 = i2 + 1
				end
				listbox[i].button.label = "<u>"..listbox_label[i].."</u>"
			end
		end
		listbox.on_modified=function()
			std_print("My dialog function on left click fired")
			local i = 1
			while listbox_label[i] ~= nil do
				listbox[i].button.label = listbox_label[i]
				i = i + 1
			end
			i = listbox.selected_index
			listbox[i].button.label = "<u>"..listbox_label[i].."</u>"

		end
		listbox.on_left_click=function()
			local i = listbox.selected_index
			local t = wesnoth.ms_since_init()
			std_print("option: "..i.." was selected, In time of: "..t)
			if last_click ~= -1 and last_click + 400 > t and last_selected == i then
				std_print("option: "..i.." was selected twice, end dialog now")
			else
				last_click = t
